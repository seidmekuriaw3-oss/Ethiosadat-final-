...
def health_check_details():
    ...
    try:
        ...
        health_status['services']['storage'] = 'accessible' if os.path.exists(upload_dir) else 'not_found'
    except Exception as e:
...
def sitemap_xml():
    try:
        ...
        for page in ['/', '/products', '/about', '/contact', '/branches', '/faq', '/shipping-info', '/returns']:
...
def test_add_to_cart(product_id):
    ...
    try:
        ...
        if session.get('user_id'):
            ...
            source = 'session'
        return jsonify({'success': True, 'product': product[1], 'source': source,
